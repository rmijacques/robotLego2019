        ...
        if(caseAct.equals(caseDest)) {
            ...
            if(nextUturn!=null) {
                if(nextUturn.isCase2()) {
                    if(tailleChemin+3<graphe.get(nextUturn)) {
                        graphe.put(nextUturn, tailleChemin+3);
                        trouver(tailleChemin+3, nextUturn, Orientation.nouvOrientApresMouv(Orientation.UTurn(orientT, caseAct.getTypeImage()), "s", nextUturn.getTypeImage()), instructions+"u\ns\n");
                    }
                }
                else if(!nextUturn.isCase2()) {
                    if(tailleChemin+3<graphe.get(nextUturn)) {
                        graphe.put(nextUturn, tailleChemin+3);
                        trouver(tailleChemin+3, nextUturn, Orientation.nouvOrientApresMouv(Orientation.UTurn(orientT, caseAct.getTypeImage()), "l", nextUturn.getTypeImage()), instructions+"u\nl\n");
                        trouver(tailleChemin+3, nextUturn, Orientation.nouvOrientApresMouv(Orientation.UTurn(orientT, caseAct.getTypeImage()), "r", nextUturn.getTypeImage()), instructions+"u\nr\n");
                    }
                }
            }
            return;
